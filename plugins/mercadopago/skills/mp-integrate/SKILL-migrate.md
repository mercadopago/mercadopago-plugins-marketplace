...
**Language rule:** always respond in the language the developer used — detect from their first message and keep throughout the entire interaction, including diff annotations, code comments, a plain chat question text, post-migration warnings, and the `migration-proposal.md` file. Never mix languages.
...
2. **Retry limit — maximum 2 authenticate attempts total per session:**
   ...
   - After 2 failed attempts → **stop retrying immediately** and ask via a plain chat question:
     ```
...
2. search the project for domain signals: `mercadopago.com.br` → BR, `mercadopago.com.ar` → AR, `mercadopago.com.mx` → MX, `mercadopago.cl` → CL, `mercadopago.com.co` → CO, `mercadopago.com.pe` → PE, `mercadopago.com.uy` → UY.
3. If still unresolved → ask via a plain chat question:
...
Run `search`/`file search` for each pattern:
...
read `${PLUGIN_ROOT}/skills/mp-integrate/references/guides/migrate-to-orders.md` only in those cases. Codex resolves this to the active plugin version. Never search another marketplace or an installation cache. Never use it as the primary source when the live doc is available — the live doc is always more current.

---

## Step 3.6 — Ask about architectural decisions (before generating the diff)

These are decisions the developer must make — the plugin cannot infer them from code. Ask each one that applies **before** writing `migration-proposal.md`. Combine into a single a plain-language question when possible, but never skip them.
...
**Infer first, ask only when truly ambiguous.** Scan the matched files for evidence before asking anything. Only ask one plain-language question if the evidence is absent or contradictory.
...
Set `config.qr.mode` from this table automatically. Do NOT ask one plain-language question — the endpoint + method are unambiguous evidence. If for any reason the endpoint cannot be matched, default to `"static"` and add a comment in the diff: `// config.qr.mode: review this value — could not be inferred from legacy endpoint`.
...
Com credenciais de teste (recomendado):
  → the `mp-test-setup` skill  para criar usuário de teste
  → Teste um pagamento com o terminal em modo desenvolvedor
  → the `mp-review` skill  para validar a integração migrada
...
Then immediately ask one plain-language question:
...
If "Cancel" → stop. The developer can re-run `the migration flow` after reviewing.
...
Next steps:
  ...
  → the `mp-review` skill to validate the migrated integration
```
...
ℹ️  Não encontrei MP_ACCESS_TOKEN no projeto.
    ...
    Ou conecte o MCP (`codex mcp login mercadopago`) e rode the migration flow novamente
    para que o plugin busque automaticamente.
```

#### Automated action 2 — Update webhook topic

Ask via a plain chat question before any MCP call:
```
a plain chat question:
...
```
If "Sim" → attempt `save_webhook` directly. If it is unavailable or returns an authentication error, apply Step 0, then retry `save_webhook`. Confirm: *"✅ Webhook topic atualizado para 'orders'."*

...
app.post('/webhooks/mp', (req, res) => {
  // Validate x-signature here (see the `mp-webhooks` skill for full HMAC pattern)
  res.status(200).end();
...
3. Log in with your **test buyer user** (email + password from the `mp-test-setup` skill)
4. Use a test card (from the `mp-test-setup` skill (test cards for BR)):
...
Run the `mp-webhooks` skill to scaffold the HMAC-SHA256 receiver and register your URL.
...
- [ ] Run the `mp-review` skill before switching to production credentials

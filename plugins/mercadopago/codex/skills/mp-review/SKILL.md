---
name: mp-review
description: Review a Mercado Pago integration with a local cross-cutting security checklist and official MCP quality tools on demand. Use after the integration is in place, or when the `mp-review` skill is invoked.
license: Apache-2.0
...
2. If it is unavailable or returns an authentication error, call `authenticate` and show the OAuth link in the developer's language.
...
Use `search`/`file search` to find:
...
- **API in use**: Payments API (`/v1/payments`) vs Orders API (`/v1/orders`). Both can coexist.
  - **If the integration uses legacy Instore APIs**, it is on the legacy path. Always include a `Needs attention` item in the Implementation Report with the file:line, and suggest the migration command: *"Run `the migration flow` to automate the migration to the Orders API."* Do **not** treat it as a Blocker (existing code still works), but flag it as forward-looking technical debt. Legacy Instore patterns to flag:
  ...
  - **If the integration uses `/v1/payments`** (Online): flag as technical debt if it is a server-side card payment (Checkout API / Marketplace). Do **not** suggest `the migration flow` for Online — migration for those products is out of scope for this plugin.
...
Call `quality_checklist`. The response defines:
...
1. Search the codebase with `search`/`read` for evidence of the item (a field, a header, a behavior).
...
| # | Check | How to verify |
|---|-------|---------------|
| 1 | Access token in `process.env` / equivalent — never hardcoded | search for `APP_USR-` / `TEST-` outside `.env*` and test fixtures |
| 2 | `.env` is in `.gitignore`, `.env.example` is **not** | read `.gitignore` |
| 3 | Webhook handler validates `x-signature` (HMAC-SHA256) | See `mp-webhooks` skill — search for `x-signature`, `x-request-id`, `createHmac` |
| 4 | HTTPS enforced for `back_url` and `notification_url` | search for `http://` in URL building |
...
| 6 | Idempotency key sent on payment/order creation | search for `X-Idempotency-Key` or SDK equivalent |
| 7 | `external_reference` set on every preference/order | search for `external_reference` |
...
| 9 | `sandbox_init_point` not used anywhere | search for `sandbox_init_point` — flag any match as a bug; Mercado Pago has no sandbox, the URL is invalid in all environments |

---

## Step 4 — Run `quality_evaluation` (when compatible)

`quality_evaluation` requires a real `payment_id`. Run it when the integration used the Payments API and produced a payment ID.

Only after the developer provides the ID, apply Step 0 immediately before this call.

| Integration uses | Action |
|------------------|--------|
| Payments API (`/v1/payments`) | Ask the developer for a recent test `payment_id`, then call `quality_evaluation` with it. |
...
1. If the developer accepts the offer, apply Step 0 immediately before calling `form_homologation` with `action="get_form"`, `product_id` (from detected product), `site_id` (from detected country), `lang`, `is_ca` (true for Checkout API / Bricks).
2. Present each step/question to the developer via a plain chat question. Collect answers.

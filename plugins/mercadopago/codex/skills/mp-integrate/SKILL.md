...
Use `${PLUGIN_ROOT}` for every bundled reference and script. Codex resolves it to the active plugin version. Never scan an installation cache, load another marketplace copy, or copy the plugin's `.mcp.json` into the developer's project.

| File | Absolute path | read when |
|---|---|---|
| `terminology-rules.md` | `${PLUGIN_ROOT}/skills/mp-integrate/references/terminology-rules.md` | **Always first** |
| `recommendation-template.md` | `${PLUGIN_ROOT}/skills/mp-integrate/references/recommendation-template.md` | **Always** — mandatory output structure |
| `guides/checkout-pro.md` | `${PLUGIN_ROOT}/skills/mp-integrate/references/guides/checkout-pro.md` | product = checkout-pro |
| `guides/checkout-api.md` | `${PLUGIN_ROOT}/skills/mp-integrate/references/guides/checkout-api.md` | product = checkout-api |
| `guides/bricks.md` | `${PLUGIN_ROOT}/skills/mp-integrate/references/guides/bricks.md` | product = bricks |
| `guides/qr.md` | `${PLUGIN_ROOT}/skills/mp-integrate/references/guides/qr.md` | product = qr |
| `guides/point.md` | `${PLUGIN_ROOT}/skills/mp-integrate/references/guides/point.md` | product = point |
| `guides/subscriptions.md` | `${PLUGIN_ROOT}/skills/mp-integrate/references/guides/subscriptions.md` | product = subscriptions |
| `guides/marketplace.md` | `${PLUGIN_ROOT}/skills/mp-integrate/references/guides/marketplace.md` | product = marketplace |
| `guides/wallet-connect.md` | `${PLUGIN_ROOT}/skills/mp-integrate/references/guides/wallet-connect.md` | product = wallet-connect |
| `guides/payouts.md` | `${PLUGIN_ROOT}/skills/mp-integrate/references/guides/payouts.md` | product = money-out (current name: Payouts) |
| `guides/smartapps.md` | `${PLUGIN_ROOT}/skills/mp-integrate/references/guides/smartapps.md` | product = smartapps |
| `guides/webhooks.md` | `${PLUGIN_ROOT}/skills/mp-integrate/references/guides/webhooks.md` | any product + webhooks |
| `products.md` | `${PLUGIN_ROOT}/skills/mp-integrate/references/products.md` | test cards per country, API reference |
...
⚠️ **IMPORTANT:** Start at Step 1.a (auto-detect SDK/client/mode). Do not
probe MCP state. Import credentials only after the developer explicitly selects
that MCP-backed operation.
...
The SDK / language is **NEVER** asked via a plain chat question. Period.
...
- Compare the installed version with that stable release. Before installing or updating, show the package, current version (or “not installed”), proposed version, and the manifest/lockfile that will change, then request explicit authorization via a plain chat question.
...
Do not ask for SDK, stack, or language: SDK selection is automatic. Do not
announce an artificial list of wizard stages before asking the next unresolved
plain-language question.

### LOCK 2 — Product → Mode availability table (NON-NEGOTIABLE)

| Product | The ONLY valid `mode` values | chat prompt behavior |
|---------|------------------------------|-----------------|
| `checkout-pro` | `preferences` (the Orders API does **not** exist for Checkout Pro) | **Skip the mode question entirely.** Do not ask one plain-language question with . Do not show "Orders API" as an option. Use `mode=preferences` silently. |
...
**If `product=checkout-pro` and you are about to render a mode question that includes `Orders API`, abort.** The Orders API is not available for Checkout Pro today. Period. Do not "future-proof" by offering it. Do not add a "Recommended" tag to it. Do not include it in any "Other" fallback.
...
### LOCK 4 — wizard context must reflect only the questions that will actually be asked

The wizard's wizard context at the top (the `□ Country  □ Product  □ Mode  ✓ Submit` line) must include **only** the dimensions that are actually still unresolved AND non-skipped per LOCK 1 and LOCK 2. Concretely:
...
```bash
node "${PLUGIN_ROOT}/scripts/validate-point-server.mjs" "{server_file}"
```
...
```bash
node "${PLUGIN_ROOT}/scripts/validate-qr-server.mjs" "{server_file}"
node "${PLUGIN_ROOT}/scripts/validate-qr-client.mjs" "{client_file}" 'data-mp-qr-cta="create-order"' "/api/qr/orders"
```
...
```bash
node "${PLUGIN_ROOT}/scripts/validate-bricks-integration.mjs" . "{brick}"
```
...
```bash
node "${PLUGIN_ROOT}/scripts/validate-subscriptions-integration.mjs" . "{subscription-model}"
```
...
```bash
node "${PLUGIN_ROOT}/scripts/validate-marketplace-integration.mjs" . "{marketplace-checkout}"
```
...
1. Confirm whether Mercado Pago enabled Wallet Connect commercially for the
   developer's application. This is not a scaffold prerequisite: when it is
   unknown or unavailable, generate only the safe structural boundary and mark
   account linking, approval-code exchange, payer-token creation, encryption,
   persistence, and order creation as `BLOCKED: active Wallet Connect agreement
   and Mercado Pago MCP login required`.
...
```bash
node "${PLUGIN_ROOT}/scripts/validate-wallet-connect-integration.mjs" .
```
...
1. Confirm whether the developer has an active SmartApps agreement with the
   Mercado Pago team. This is not a scaffold prerequisite: when it is not
   confirmed, generate only the safe Android project structure and mark the
   private kit, terminal and payment portions as `BLOCKED: active SmartApps
   agreement and Mercado Pago MCP login required`.
2. Do not use MCP as a prerequisite for this scaffold. Use the bundled guide
   and public `llms.txt` when available to generate the safe Android project
   structure. If the current private-kit contract is not available, label the
   AAR, terminal and payment-operation portions as blocked; do not authenticate
   automatically and do not invent those details.
...
```bash
node "${PLUGIN_ROOT}/scripts/validate-smartapps-integration.mjs" . "{main|mini}" "{own|third-party}"
```
...
1. Never scaffold a `disbursements` API, Advanced Payments, Marketplace split,
   ...
   to Payouts.
2. Resolve the contract from bundled country guidance before writing. Argentina
   uses the current batch Payouts contract; Brazil uses the current single
   Transaction Intent contract. For another country, generate the privileged
   service boundary only and mark the country-specific request contract as
   `BLOCKED: verified Payouts country contract and Mercado Pago MCP login
   required`; do not query MCP or stop the structural scaffold.
...
```bash
node "${PLUGIN_ROOT}/scripts/validate-payouts-integration.mjs" . "{AR|BR}"
```
...
**You MUST run this step before any a plain-language question.** Every dimension that resolves here is removed from the wizard. The developer should only be asked about dimensions that genuinely cannot be inferred. **Skipping the auto-detection and asking the developer anyway is the single most common mistake — do not do it.**
...
|-----------|--------------------------------|----------------|-------------------|----------|
| `product` | If agent resolved from message keywords → **use it, skip question** | read from `.mp-integrate-progress.md` | — | a plain chat question |
| `country` | If agent resolved from message keywords → **use it, skip question** | read from `.mp-integrate-progress.md` | **None — do not grep for country** (unreliable, costs tokens) | a plain chat question. **Persist the answer**. |
| `sdk` | — | — | **MUST run `file search` for**: `package.json`→node, `pyproject.toml`/`requirements.txt`→python, `pom.xml`/`build.gradle*`→java, `composer.json`→php, `Gemfile`→ruby, `*.csproj`/`Program.cs`→dotnet, `go.mod`→go. Single match → resolved. Multiple matches → choose the manifest in the active application area. No match → default to node. | **Never ask which SDK**; only ask authorization before an install/update. |
| `client` | — | — | Inspect `package.json` deps: `react`/`next`→react, `react-native`/`expo`→react-native, `*.xcodeproj`→ios, Android `build.gradle`→android, `pubspec.yaml`→flutter. Single match → resolved. | a plain chat question (only if product has client component AND ambiguous) |
| `lang` | — | — | Derive from country (BR→pt, others→es). | Almost never asked — defaulted from country |
| `mode` | — | read from progress file | `search` for `/v1/orders`/`order.create`→orders; `/v1/payments`/`payment.create`→payments; `/checkout/preferences`/`preference.create`→preferences. Product Matrix may pin to single value (e.g. checkout-pro → always preferences). | a plain chat question (only when matrix allows >1 AND grep didn't disambiguate) |

**Concrete order of operations for the wizard (INFER FIRST, ASK LAST):**

0. **Check for legacy Instore APIs in repo (before any question).** Run `search` for legacy QR/Point patterns:
   ...
   If any found AND the project is not already fully on `/v1/orders`, ask **once** via a plain chat question:
   ...
   - Options: `"Yes, migrate first (the migration flow)"` / `"No, scaffold the new integration"`
   - If "Yes" → stop and instruct: *"Run `the migration flow` to migrate the existing integration, then come back to scaffold the new one."*
   - If "No" → continue normally. Do NOT suggest migration again in this session.
   - **Skip this check entirely** if `developer-provided context` already contains `migrate`.
...
3. **Do NOT grep for country.** Country is asked via a plain chat question unless already resolved by steps 1–2. No locale-string grep, no `mercadopago.com.<tld>` grep, no `currency_id` grep — they cost tokens and produce wrong matches.
4. Run `file search` over the manifest patterns. **If a single SDK manifest matches, the SDK is RESOLVED — do NOT ask.** The official Mercado Pago SDK for the detected language is the one used. Never propose a third-party SDK.
5. If the product needs a client, run `file search`/`search` on manifest deps. If a single client matches, **client resolved**. Skip the client question.
6. Default `lang` from country. Skip the lang question.
7. Now — and only now — ask one plain-language question for whatever is still missing, one tool call at a time, in the order defined in Step 1.b. After each answer, **persist it** to `.mp-integrate-progress.md`.

**Known MCP limitation — country resolution:** The Mercado Pago MCP does not currently expose a tool that returns the developer's `site_id` (neither `application_list` nor `quality_checklist` nor `notifications_history` carry the country in their response). The OAuth access token would let us call `GET https://api.mercadopago.com/users/me` directly, but the token is held by the MCP server and is not exposed to the plugin client. Until MP ships a new MCP tool (e.g. `current_user_info` or a generic `proxy_request`), country resolution is **just**: read `.mp-integrate-progress.md` if it has a country, otherwise ask via a plain chat question and persist. **Do not** waste tokens grepping the repo for country signals (locales, URLs, `currency_id`, `site_id`, app-name heuristics) — they don't pay off, and asking the developer once is cheaper and more reliable.

### Step 1.a.iii — Confirm everything that was auto-resolved (mandatory)

After auto-resolving, render a single confirmation block listing every value that was inferred (not just country — also SDK, client, and any other dimension that came from the repo). Then, **before any wizard question**, ask the developer with one plain-language question:
...
- Options:
  ...
  - `"No, let me correct"` — drop the user-selectable auto-resolved values back into the wizard queue. Do not add SDK/language to that queue: re-run repository detection for them and explain the selected official SDK without presenting an SDK question.
...
Only when the product/mode combination is absent or genuinely ambiguous in the bundled sources, generate the structural scaffold with the mode-sensitive operation marked as a limitation. Do not call `search_documentation` or authenticate before scaffold. The developer may explicitly request a live documentation lookup afterwards. Never run it for `checkout-pro` — LOCK 2 already forbids Orders for that product.
...
**`country` will commonly end up in this list.** Today the MCP does not return `site_id`, so unless repo signals or persisted state resolved it in 1.a, you will need to ask. Ask which supported country applies (AR, BR, MX, CO, CL, PE, or UY). After the answer, persist it to `.mp-integrate-progress.md`.
...
→ then immediately the a plain-language question for `product`. The developer picks. Then ≤1 line confirmation. Then the next a plain chat question. And so on.
...
Resolve documentation in **tiers**. The official `llms.txt` per country is always fresh — fetch it first when it is accessible. Fall back gracefully if unreachable.

**MCP is never a prerequisite for structural scaffold.** Do not call
`authenticate`, `application_list`, `search_documentation`, or any other MCP
tool before generating a safe scaffold. A failed WebFetch or unavailable MCP
must not stop Checkout Pro, Checkout API, Bricks, Point, QR, Subscriptions,
Marketplace, Wallet Connect, Payouts, or SmartApps from receiving the structure
that is supported by bundled references. State any missing product detail as a
clear limitation instead of inventing a payload, endpoint, SDK artifact, or
commercial contract.
...
| 2 | `${PLUGIN_ROOT}/skills/mp-integrate/references/products.md` (bundled) | none | **Always** — product guides, API reference, code snippets, country-specific test cards. |
| 3 | `search_documentation` | MCP OAuth | Optional fallback, only after the scaffold when the developer explicitly requests live documentation for a detail not covered by tiers 1–2. |
...
**When tier 3 is explicitly requested after scaffold:** attempt
`search_documentation` directly. If it is unavailable or returns an
authentication error, do not authenticate automatically and do not block the
scaffold. State the unavailable detail and offer `codex mcp login mercadopago`
only when the developer chooses to retrieve live documentation. Do not call
`application_list` as a probe.

### MCP-only account and live-state operations

The following operations must return `BLOCKED: Mercado Pago MCP login required
— run codex mcp login mercadopago` when MCP is unavailable: application or
credential lookup; test-user creation or funding; webhook registration or
delivery lookup; creation or lookup of real Mercado Pago resources; commercial
OAuth, Wallet Connect enablement, SmartApps private-kit access, or financial
operations. They are never part of structural scaffold.

### Tier 3 — MCP query templates (fallback)

Build 1–3 targeted queries and call `search_documentation` with each. Use `language` from the resolved doc language.
...
{paymentStatus === 'approved' && (
  <div className="checkout-success">
    ...
    <p>Next step: run <code>the `mp-review` skill</code> to validate your integration before going live.</p>
  </div>
)}
...
## 5. Webhook receiver
> Webhook validation is handled by the `mp-webhooks` skill — invoke it next, or run the `mp-webhooks` skill to scaffold the receiver with HMAC validation.

## 6. Test
- Get test credentials and test users via the `mp-test-setup` skill (or run the `mp-test-setup` skill).
- Test cards for the country: see `references/products.md` (AR/BR/MX/CO/CL) or query MCP for others.
- After your first successful test payment → run the `mp-review` skill (**step 6 of 7 — MANDATORY before production**).
...
2.5. **`checkout-pro` and `checkout-api` — Mandatory checkout CTA discovery** (skip for all other products):
   ...
   **Resolve the target:**
   ...
   5. If it returns `status=ambiguous`, ask via a plain chat question which candidate is the checkout CTA (maximum four candidates). A target must be selected before scaffolding continues.
   6. If it returns `status=not_found`, inspect the application's actual cart/order/product entry screens and ask via a plain chat question where the new checkout CTA should be inserted (maximum four concrete file/location choices). After the developer chooses, set `cta_status=create` and use that exact location.
...
3. **`checkout-api` only — Always create a separate checkout screen in a new file** — generate the client form from Step 4 using the project's architecture. For `checkout-pro`, skip directly to Step 3.5; Checkout Pro redirects to Mercado Pago and must not create a local payment-form screen.
   ...
   **Checkout API secure-field integrity — mandatory and application-agnostic:**
   ...
   node "${PLUGIN_ROOT}/scripts/validate-checkout-screen.mjs" "{checkout_screen_file}" "{server_or_config_route_file}" "{cta_file}"

   # framework-public-config strategy: no server config file is needed
   node "${PLUGIN_ROOT}/scripts/validate-checkout-screen.mjs" "{checkout_screen_file}" "" "{cta_file}"
   ```
...
3.5. **`checkout-pro` and `checkout-api` — Always wire the resolved CTA:**
   ...
   **Mandatory verification after editing:**
   ...
   6. Run the deterministic CTA acceptance check and fix any failure before continuing:
      ...
      ```bash
      node "${PLUGIN_ROOT}/scripts/validate-checkout-cta.mjs" "{product}" "{cta_file}" "{checkout_screen_destination_or_preference_route}"
      ```
      ...
      ```bash
      node "${PLUGIN_ROOT}/scripts/validate-checkout-pro-server.mjs" "{server_file}"
      ```
...
1. **Run the `mp-webhooks` skill** to add the webhook receiver (HMAC validation included).
2. **Run the `mp-test-setup` skill** to create a test user and load funds.
2.5. **For Orders API products (checkout-api / qr / point — ALL countries):** Ask via a plain chat question before listing next steps:
   - Question: *"Orders API requires a test buyer user to process payments. Do you want me to create one now?"*
   - Options: `"Yes, create test user now"` → invoke `mp-test-setup` skill inline; `"No, I'll do it later"` → show reminder: *"⚠️ Run the `mp-test-setup` skill before testing — Orders API returns 422 without a test user."*
...
3. **Run the `mp-review` skill — MANDATORY before going to production.** This is **step 6 of 7** in your integration journey. The auto-checker validates your integration automatically after the first payment. Do **not** switch to production credentials (step 7) until the `mp-review` skill passes.

> You are at step 6 of 7. After the `mp-review` skill passes, the only remaining step is switching to production credentials.
...
### checkout-api
- **⛔ Orders API requires a test user buyer** — `422 unprocessable_content` if you use an arbitrary email. Run the `mp-test-setup` skill **before** testing. This applies to ALL countries.
...
- It does **not** validate webhooks. Use the `mp-webhooks` skill (or the `mp-webhooks` skill).
- It does **not** create test users. Use the `mp-test-setup` skill (or the `mp-test-setup` skill).
- It does **not** evaluate integration quality. Use the `mp-review` skill (or the `mp-review` skill).

...
Use the `mp-test-setup` skill to obtain test cards for `{country}`. Quick
...
1. Run the `mp-test-setup` skill to create a buyer test user
...
- [ ] **Test user created** via the `mp-test-setup` skill (blocker for Orders API)
...
- [ ] Run the `mp-review` skill before production

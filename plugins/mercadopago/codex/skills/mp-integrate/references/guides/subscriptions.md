...
Ask for this contract with `a single chat question` only when it cannot be inferred from the application or the developer's request. A subscription created without `preapproval_plan_id` cannot later be migrated to a plan.
...
```bash
node "${PLUGIN_ROOT}/scripts/validate-subscriptions-integration.mjs" . "{contract}"
```

...
def is_mercadopago_project(start_dir: str) -> bool:
    ...
    for _ in range(32):
        if os.path.isfile(os.path.join(current, ".codex", "mercadopago.local.md")):
            return True
...
def read_settings() -> Dict[str, str]:
    """Read per-project .codex/mercadopago.local.md frontmatter."""
    settings_path = os.path.join(os.getcwd(), ".codex", "mercadopago.local.md")
...
def block(message: str) -> None:
    ...
    # Codex reads this structure to deny the proposed Bash operation. stderr
    # keeps the refusal clear in runtimes that only surface process output.
    print(json.dumps({
        "hookSpecificOutput": {
